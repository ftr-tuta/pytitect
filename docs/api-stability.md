...
`pytitect.event_sourcing` remain Preview in the stable Pytitect 1.6.0 package. A stable package version
does not promote these APIs to Stable. Versioned wire bundles are closed independently from Python
API maturity. Preview APIs require an explicit changelog entry and migration guidance when changed.
